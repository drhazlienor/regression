...
```{r}
#install.packages(setdiff(c("haven", "psych", "tidyverse", "broom", "gtsummary", "ggplot2", "GGally", "lmtest", "rsq", "car", "sjplot", "corrplot"), rownames(installed.packages())))
```
...
Since the **p-value is highly significant**, we **reject the null hypothesis** of constant variance.
This means the **residuals do not have constant variance** — **heteroscedasticity is present**.
